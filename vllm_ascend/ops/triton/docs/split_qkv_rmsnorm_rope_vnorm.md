...
- Fusion-pass test: `tests/e2e/pull_request/one_card/compile/test_graphex_qkvnorm_rope_fusion.py`, which compiles the pre-attention chain for both Gemma4 shapes and asserts the graph holds `qkv_rmsnorm_rope_vnorm` and no leftover `npu_rms_norm` or `npu_rotary_embedding`. It also asserts the two shapes produce distinct pattern keys, since a shared key would silently drop the second registration.
